...
--      BY A POSITIONAL COMPONENT ASSOCIATION.
     ...
     DECLARE
          ...
          B1 : A_R2 := NEW R2'(-2, -3, D => -4,
                               A => -1);               -- ERROR: B1.
          B2 : CONSTANT A_R2 := NEW R2'(1, 2, 3, B => 2);  -- ERROR: B1.
          B3 : A_R2;

     BEGIN

          B3 := NEW R2'(4, C => 2, D => 1, A => 1);    -- ERROR: B1.
...
--   C) A CHOICE THAT IS NOT THE IDENTIFIER OF A COMPONENT.
     ...
     DECLARE
          ...
          C1 : A_R1 := NEW R1'(1, 2, 3, D => 4);       -- ERROR: C1.
          C2 : CONSTANT A_R1 := NEW R1'(1, 2, D => 3,  -- ERROR: C1.
                                        C => 4);
          C3 : A_R1;
          C4 : A_R3 := NEW R3'(1, C => 2);             -- ERROR: C1.
          C5 : CONSTANT A_R3 := NEW R3'(2, B => -1);   -- ERROR: C1.
          C6 : A_R3;

     BEGIN

          C3 := NEW R1'(A => 1, D => 4,                -- ERROR: C1.
                        C => 3, B => 2);
          C6 := NEW R3'(1, C => -1);                   -- ERROR: C1.
...
--      OF DIFFERENT TYPES.
     ...
     DECLARE
          ...
          D1 : A_R4 := NEW R4'(A => -1, OTHERS => -2); -- ERROR: D1.
          D2 : CONSTANT A_R4 := NEW R4'(A | B => -2,   -- ERROR: D1.
                                        C => -3);
          D3 : A_R4;

     BEGIN

          D3 := NEW R4'(C => 1, OTHERS => 4);          -- ERROR: D1.
...
--      CHOICE DOES NOT REPRESENT AT LEAST ONE COMPONENT OF THE RECORD.
     ...
     DECLARE
          ...
          E1 : A_R1 := NEW R1'(1, 2, 3, OTHERS => 4);  -- ERROR: E1.
     ...
     BEGIN
          ...
          E3 := NEW R1'(1, 2, C => 3, OTHERS => 4);    -- ERROR: E1.
...
--      SUBTYPE.
     ...
     DECLARE
          ...
          F1 : A_R1 := NEW R1'(1, 2);                  -- ERROR: F1.
          F2 : CONSTANT A_R1 := NEW R1'(B => 2, C => 3);   -- ERROR: F1.
          F3 : A_R1;
          F4 : A_R5(1) := NEW R5'(1, 5, 4);            -- ERROR: F1.
          F5 : CONSTANT A_R5(2) := NEW R5'(2, 3);      -- ERROR: F1.
          F6 : A_R5(2);

     BEGIN

          F3 := NEW R1'(1, C => 3);                    -- ERROR: F1.
          F6 := NEW R5'(2, 1, 2, 3);                   -- ERROR: F1.
...
--   G) A RANGE USING COMPONENT NAMES FOR LOWER AND UPPER BOUNDS.
     ...
     DECLARE
          ...
          G1 : A_R2 := NEW R2'(A .. C | D => 1);       -- ERROR: G1.
          G2 : CONSTANT A_R2 := NEW R2'(A .. D => -1); -- ERROR: G1.
          G3 : A_R2;

     BEGIN

          G3 := NEW R2'(A .. B => -1, C .. D => -2);   -- ERROR: G1.
